...
| P2 ✅ | Literature review notes; `src/chunkers/` package behind CLI flag (default `legacy`) — **all five candidates implemented (C0 legacy, C1 overlap, C3 recursive_paragraph, C4 semantic, C5 document_structure) with unit tests**; logging of call counts/tokens; optional linter/type-check proposal | Baseline byte-identical outputs vs pre-change run on small sample |
